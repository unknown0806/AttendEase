...
from db import db
from models import User, Student, ClassSubject, Subject, Attendance, TeacherSubject, Period
...
def results():
    user = get_current_user()
    student = Student.query.filter_by(user_id=user.user_id).first()
    return render_template('student/results.html', user=user, student=student)
